...
//    double delta = input.delta;
    ...
    */
    ...
    // The rank is used to seed the random number generator
    for (int i = 0; i < World_size; ++i) {
        rrp[i] = gsl_rng_alloc(TT);
        seed_rrp[i] = i;
        gsl_rng_set(rrp[i], seed_rrp[i]);
...
//    MPI_Reduce(full_abszsum1, total_hargzsum1, N_slice, MPI_DOUBLE, MPI_SUM, 0, MPI_COMM_WORLD);
    ...
    if (rank == 0) {
        ...
        */
        ...
        printf("Time        abszsum1   argzsum1   realsum    habszsum1   hargzsum1   hrealsum\n");
        fprintf(stream, "Time   abszsum1   argzsum1   realsum   habszsum1   hargzsum1   hrealsum\n");
        for (int i = 0; i < N_slice; ++i) {
//        for (int j = 0; j < Ncut; ++j) {
            int j = 0;
            printf("%lf    %lf   %lf   %lf   %lf   %lf   %lf\n", TSLICE * (i + 1),
                   (total_abszsum1[i] / Nsample), /*pow(variance[i] / Nsample, 0.5),*/ (total_argzsum1[i] / Nsample),
                   realsum[i][j] / Nsample,
                   (total_habszsum1[i] / Nsample), (total_hargzsum1[i] / Nsample), hrealsum[i][j] / Nsample);
            fprintf(stream, "%.2lf   %lf   %lf   %lf   %lf   %lf   %lf\n", TSLICE * (i + 1),
                    (total_abszsum1[i] / Nsample), /*pow(variance[i] / Nsample, 0.5),*/ (total_argzsum1[i] / Nsample),
                    realsum[i][j] / Nsample,
                    (total_habszsum1[i] / Nsample), (total_hargzsum1[i] / Nsample), hrealsum[i][j] / Nsample);
//        }
...
//        if (input.Regression_test == 1) {
        if(Regression_test == 1){
            ...
            // If abszsum1 goes out of bounds at any point up to T = 3 the test is failed
            if (find(Regression.begin(), Regression.end(), 0) != Regression.end()) {
